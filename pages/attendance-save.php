...
try {
    ...
    foreach ($records as $rec) {
        ...
        $upd = $db->prepare("UPDATE attendance SET status=?, remarks=?, class_name=?, period=?, marked_by=?
                              WHERE student_id=? AND attendance_date=?");
        $upd->execute([$status, $remark, $class_name, $period, $teacher_id, $sid, $date]);

        if ($upd->rowCount() === 0) {
            $ins = $db->prepare("INSERT INTO attendance (student_id, class_name, attendance_date, period, status, remarks, marked_by)
                                 VALUES (?,?,?,?,?,?,?)");
            $ins->execute([$sid, $class_name, $date, $period, $status, $remark, $teacher_id]);
        }
        $saved++;
    }
