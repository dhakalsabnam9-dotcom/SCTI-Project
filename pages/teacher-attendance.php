...
.already-marked-banner{background:#fef3c7;border:2px solid #f59e0b;border-radius:10px;padding:12px 18px;margin-bottom:16px;display:none;align-items:center;gap:10px;font-size:13px;font-weight:600;color:#78350f}
.already-marked-banner i{font-size:18px;color:#f59e0b;flex-shrink:0}
...
.toast{min-width:280px;max-width:380px;padding:14px 18px;border-radius:12px;font-size:13px;font-weight:600;display:flex;align-items:center;gap:10px;box-shadow:0 6px 24px rgba(0,0,0,.18);pointer-events:all;animation:slideIn .3s ease;transition:opacity .4s ease,transform .4s ease}
.toast.ok{background:#065f46;color:#d1fae5;border-left:5px solid #10b981}
.toast.err{background:#7f1d1d;color:#fee2e2;border-left:5px solid #ef4444}
.toast.warn{background:#78350f;color:#fef3c7;border-left:5px solid #f59e0b}
...
<div class="wrap">
  <div id="alreadyMarkedBanner" class="already-marked-banner">
    <i class="fa fa-exclamation-triangle"></i>
    <span id="alreadyMarkedMsg">Attendance already marked for this date. You can update it below.</span>
  </div>
...
<footer>&copy; 2025 SCTI - Teacher Portal</footer>
...
function loadStudents() {
  ...
  if (!date || !subj || !sem) { showToast('<i class="fa fa-exclamation-circle"></i> Please select date, program and semester.', 'err'); return; }
  ...
  document.getElementById('alreadyMarkedBanner').style.display = 'none';

  var cls = subj + ' ' + sem;
  Promise.all([
    fetch('attendance-data.php?action=students&class=' + encodeURIComponent(cls)).then(function(r){ return r.json(); }),
    fetch('attendance-data.php?action=existing&date=' + encodeURIComponent(date) + '&class=' + encodeURIComponent(cls)).then(function(r){ return r.json(); })
  ]).then(function(results) {
    var sRes = results[0], eRes = results[1];
    if (!sRes.success) { showToast('<i class="fa fa-times-circle"></i> ' + (sRes.message || 'Failed to load students'), 'err'); return; }
    allStudents = sRes.students || [];
    existingMap = (eRes.success && eRes.records) ? eRes.records : {};

    var banner = document.getElementById('alreadyMarkedBanner');
    var btnSave = document.getElementById('btnSave');
    if (eRes.already_marked) {
      document.getElementById('alreadyMarkedMsg').textContent =
        'Attendance already marked for this date (' + eRes.count + ' record(s)). Existing statuses are pre-filled — you can update and save again.';
      banner.style.display = 'flex';
      btnSave.innerHTML = '<i class="fa fa-sync"></i> Update Attendance';
    } else {
      banner.style.display = 'none';
      btnSave.innerHTML = '<i class="fa fa-save"></i> Save Attendance';
    }

    renderTable(allStudents);
    updateStats();
    btnSave.disabled = allStudents.length === 0;
    document.getElementById('saveInfo').textContent = allStudents.length + ' students - ' + subj + ' / ' + sem;
  }).catch(function(e) { showToast('<i class="fa fa-times-circle"></i> Network error: ' + e.message, 'err'); });
}
...
function renderTable(list) {
  ...
  var html = '<table><thead><tr><th>#</th><th>Student</th><th>Status</th><th>Late Reason</th><th>Remarks</th></tr></thead><tbody>';
  list.forEach(function(s, i) {
    ...
    html += '<tr id="row_' + s.id + '">'
      ...
      + '<div class="student-meta">' + esc(s.student_id) + ' | ' + esc(s.course||'') + (s.semester ? ' ' + esc(s.semester) : '') + '</div></td>'
      ...
      + '<input type="text" id="lrInput_' + s.id + '" placeholder="Reason for being late..." value="' + esc(lr) + '"></div></td>'
...
function updateStats() {
  ...
  allStudents.forEach(function(s) {
    ...
    if (val==='present') p++; else if (val==='absent') a++; else if (val==='late') l++;
  });
...
function saveAttendance() {
  ...
  var sem  = document.getElementById('attPeriod').value.trim();
  var cls  = subj + ' ' + sem;
  if (!date || !subj || !sem) { showToast('<i class="fa fa-exclamation-circle"></i> Date, program and semester are required.', 'err'); return; }
  if (!allStudents.length) { showToast('<i class="fa fa-exclamation-circle"></i> No students loaded.', 'err'); return; }
  ...
  .then(function(d) {
    ...
    if (d.success) {
      showToast('<i class="fa fa-check-circle"></i> ' + d.message, 'ok');
      resetForm();
    } else {
      var msg = d.message || '';
      var isDup = msg.toLowerCase().includes('duplicate') || msg.includes('1062') || msg.toLowerCase().includes('integrity constraint');
      if (isDup) {
        showToast('<i class="fa fa-exclamation-triangle"></i> Attendance already marked. Load students again to update existing records.', 'warn');
      } else {
        showToast('<i class="fa fa-times-circle"></i> ' + (msg || 'Failed to save attendance.'), 'err');
      }
    }
  })
  .catch(function(e) {
    btn.disabled = false;
    btn.innerHTML = '<i class="fa fa-save"></i> Save Attendance';
    showToast('<i class="fa fa-times-circle"></i> Network error: ' + e.message, 'err');
  });
}
...
function resetForm() {
  ...
  document.getElementById('attPeriod').value  = '';
  document.getElementById('searchStudent').value = '';
  document.getElementById('alreadyMarkedBanner').style.display = 'none';
  document.getElementById('tableWrap').innerHTML = '<div class="empty-state"><i class="fa fa-calendar-check"></i><p>Select a date and class, then click <strong>Load Students</strong></p></div>';
  document.getElementById('btnSave').disabled = true;
  document.getElementById('btnSave').innerHTML = '<i class="fa fa-save"></i> Save Attendance';
  document.getElementById('saveInfo').textContent = 'No attendance loaded';
  document.getElementById('stTotal').textContent   = '0';
  document.getElementById('stPresent').textContent = '0';
  document.getElementById('stAbsent').textContent  = '0';
  document.getElementById('stLate').textContent    = '0';
}
