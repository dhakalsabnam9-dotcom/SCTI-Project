...
try {
    ...
    // Add semester column if it doesn't exist yet (for existing installs)
    try { $db->exec("ALTER TABLE grades ADD COLUMN semester VARCHAR(30) DEFAULT NULL"); } catch(Exception $ex) {}
    ...
    foreach ($records as $rec) {
        ...
        $stmt = $db->prepare("INSERT INTO grades (student_id, student_db_id, subject, exam_type, semester, internal_marks, external_marks, marked_by)
            VALUES (?,?,?,?,?,?,?,?)
            ON DUPLICATE KEY UPDATE internal_marks=VALUES(internal_marks), external_marks=VALUES(external_marks), semester=VALUES(semester), updated_at=NOW()");
        $stmt->execute([$sid, $sdbid, $subject, $exam_type, $semester ?: null, $int, $ext, $_SESSION['user_id']??null]);
        $saved++;
    }
